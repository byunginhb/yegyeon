-- ============================================================
-- 예견 (YEGYEON) — 자동 5분 마켓 시간 감쇠 배당
--   문제: 시작가가 5분 내내 노출 + parimutuel이라, 막판(4:59)에 현재가를 보고
--          거의 확정된 방향에 걸면 초반 베터의 돈을 가져가는 정보우위 악용 가능.
--   수정: 베팅 시점 가중치(weight = 남은시간/5분)를 shares에 반영.
--          정산 분배는 shares(=amount×weight) 비례, 전체 풀은 실제 금액 합.
--          → 늦게 걸수록 배당↓. 줄어든 몫은 분배되지 않고 소각(포인트 싱크).
--   영향: 일반 마켓은 weight=1(shares=amount)이라 동작 불변.
--          auto_kind='btc_5m' 마켓에만 감쇠 적용.
-- ============================================================

-- ----------------------------------------------------------------
-- 1. place_bet — btc_5m 마켓이면 shares = amount × (남은시간/5분)
-- ----------------------------------------------------------------
...
BEGIN
  ...
  -- 시간 감쇠 가중치: 자동 5분 마켓만 적용 (그 외 weight=1)
  -- weight = 남은시간/5분, [0.01, 1] 범위
  IF v_market.auto_kind = 'btc_5m' THEN
    v_weight := LEAST(1.0, GREATEST(0.01,
      EXTRACT(EPOCH FROM (v_market.close_date - now())) / 300.0));
  ELSE
    v_weight := 1;
  END IF;
  v_shares := p_amount * v_weight;
...
-- ----------------------------------------------------------------
-- 2. resolve_market — 승자 분배를 shares(시간가중) 비례로
--    (총 풀은 실제 금액 합, 줄어든 몫은 소각. 일반 마켓은 shares=amount라 불변)
-- ----------------------------------------------------------------
...
BEGIN
  ...
  IF v_winners_count = 0 THEN
    ...
    END LOOP;
  ELSE
    -- 1) 베팅별 payout 계산: shares(시간가중) 비례로 전체 풀 분배
    FOR v_bet IN
      SELECT b.id, b.shares, b.outcome
        FROM public.bets b
